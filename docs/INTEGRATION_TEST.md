...
Before running, make sure no test pods are running in the cluster. For example, if you rerun the test, the test would not know about test pods that may have been left over from a previous run. Enusre you check for any test pods before starting. The following is an example of the command:
