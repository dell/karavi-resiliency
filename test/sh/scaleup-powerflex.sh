...
if [ "$ISVIRTUALIZATION" = true ]; then
	...
	wait_on_running
	sh ../sh/nway.sh --ns vxflexos --bounceipseconds $BOUNCEIPTIME --maxiterations 12 --timeoutseconds 600 --node-user $NODE_USER --password $PASSWORD
	fi
	...
	cd ../podmontest; sh insv.sh --instances "$instances" --nvolumes $NVOLUMES --storage-class $STORAGECLASS; cd $CWD
	wait_on_running
	sh ../sh/nway.sh --ns vxflexos --bounceipseconds $BOUNCEIPTIME --maxiterations 12 --timeoutseconds 900 --node-user $NODE_USER --password $PASSWORD
	fi
	...
	cd ../podmontest; sh insv.sh --instances "$instances" --nvolumes $NVOLUMES --storage-class $STORAGECLASS; cd $CWD
	wait_on_running
	sh ../sh/nway.sh --ns vxflexos --bounceipseconds $BOUNCEIPTIME --maxiterations 12 --timeoutseconds 1300 --node-user $NODE_USER --password $PASSWORD
	fi
